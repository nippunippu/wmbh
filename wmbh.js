...
if (chat) {
  ...
  console.log("messagebuffer not found");
	...
	function updateDescription() {
	    const oldDesc = document.querySelector('.wmb-description-container');
	    if (oldDesc) oldDesc.remove();
	
	    const desc = document.createElement('div');
	    desc.className = 'wmb-description-container';
	    desc.innerHTML = DESCRIPTION_CONTENT;
	
	    const poll = document.querySelector('#pollwrap');
	    const parent = poll ? poll.parentNode : document.querySelector('#leftpane');
	    if (parent) {
	        parent.insertBefore(desc, poll || parent.firstChild);
	    }
	}
	...
	document.addEventListener('click', (e) => {
	    if (e.target.id === 'cs-jssubmit') setTimeout(updateDescription, 500);
	});
